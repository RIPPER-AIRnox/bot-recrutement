...
async function createSogDossier(interaction) {
  ...
  const channel = await guild.channels.create({
    ...
    permissionOverwrites: [
      ...
      // ➕ ajoute ici le rôle recruteur si besoin
    ],
  });

  // Bouton "Finaliser mon dossier"
  ...
  const embed = {
    ...
    footer: {
      text: "CIR - Région d'Île-de-France  • " + new Date().toLocaleString("fr-FR"),
    },
  };
...
async function createEgavDossierFromModal(interaction, { nom, prenom, age, motivation }) {
  const guild = interaction.guild;
